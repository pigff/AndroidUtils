package com.linjianlin.utils.other;
...
public class OvalProgrossView extends View {
    ...
    public OvalProgrossView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.OvalProgressView_ovalColor) {
                mOvalColor = a.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.OvalProgressView_fixTextColor) {
                mfixTextColor = a.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.OvalProgressView_progressTextColor) {
                mProgressColor = a.getColor(attr, Color.RED);

            } else if (attr == R.styleable.OvalProgressView_progressTextSize) {
                mProgressTextSize = a.getDimensionPixelSize(attr, sp2px(50));

            } else if (attr == R.styleable.OvalProgressView_fixTextSize) {
                mFixTextSize = a.getDimensionPixelSize(attr, sp2px(40));

            } else if (attr == R.styleable.OvalProgressView_ovalWidth) {
                mOvalWidth = a.getDimensionPixelSize(attr, sp2px(5));

            } else if (attr == R.styleable.OvalProgressView_progressText) {
                mProgressText = a.getString(attr);
                mOvalProgress = Math.abs(Float.valueOf(mProgressText));

            } else if (attr == R.styleable.OvalProgressView_fixText) {
    ...
    public void setmOvalProgress(float mOvalProgress) {
        this.mOvalProgress = Math.abs(mOvalProgress);
        isStop = false;
        postInvalidate();
        mProgress = 0;
        if (mOvalProgress >= 0) {
